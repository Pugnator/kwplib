...
namespace KWP2000
{
  ...
  unique_ptr<kwp_message> kwpClient::process_command(const service_mnemonic &cmd, const initializer_list<uint8_t> &params)
  {
    DEBUG_LOG("\r\n====================\r\n");
  ...
  ECU_identification_table *kwpClient::read_ECU_identification(const identificationOption &options)
  {
    ...
    DEBUG_LOG("Size: {0:#02x} [{1}]\r\n", resp->length, resp->length);
    for (auto i = 1; i < resp->length; i++)
    {
      DEBUG_LOG("{0}", resp->data[resp->header.type + i]);
    }

    memcpy(ECU_ID.data, &resp->data[resp->header.type + 1], sizeof(ECU_identification_table));
  ...
  unique_ptr<RLI_ASS_tab> kwpClient::read_rli_ass()
  {
    auto resp = process_command(KWP_RDBLI, {RLI_ASS});
    if (!resp)
    {
      return unique_ptr<RLI_ASS_tab>{};
    }
    unique_ptr<RLI_ASS_tab> rlitab(new RLI_ASS_tab);
    uint8_t *payload = &resp->data[resp->header.type + 1];
    memcpy(rlitab->tab, payload, sizeof(rlitab->tab));
    for (auto i = 0; i < 36; i++)
    {
  ...
  unique_ptr<RLI_OLT_tab> kwpClient::read_olt_diag()
  {
    auto resp = process_command(KWP_RDBLI, {RLI_OLT});
    if (!resp)
    {
      return unique_ptr<RLI_OLT_tab>{};
    }
    unique_ptr<RLI_OLT_tab> rlitab(new RLI_OLT_tab);
    uint8_t *payload = &resp->data[resp->header.type + 1];
    memcpy(rlitab->tab, payload, sizeof(rlitab->tab));
    for (auto i = 0; i < 81; i++)
    {
  ...
  unique_ptr<RLI_TRS_tab> kwpClient::read_trs_diag()
  {
    auto resp = process_command(KWP_RDBLI, {RLI_TRS_LONG});
    if (!resp)
    {
      return unique_ptr<RLI_TRS_tab>{};
    }
    unique_ptr<RLI_TRS_tab> rlitab(new RLI_TRS_tab);
    uint8_t *payload = &resp->data[resp->header.type + 1];
    memcpy(rlitab->tab, payload, sizeof(rlitab->tab));
    for (auto i = 0; i < 81; i++)
    {
